(function (angular) {
  ...
  module.controller("RegistrationPayments", [
    "$scope",
    "RegistrationPaymentsService",
    "$window",
    function ($scope, RegistrationPaymentsService, $window) {
      $scope.data = {
        payments: [],
        editPayment: null,
      ...
      RegistrationPaymentsService.find({
      ...
      $scope.loadMore = function () {
        return;
        var search = $scope.data.search;
        var page = $scope.data.apiMetadata.page
          ? parseInt($scope.data.apiMetadata.page) + 1
          : 1;
        var status = $scope.data.status ? $scope.data.status : null;
        var from = $scope.data.filterDateFrom;
        var to = $scope.data.filterDateTo;
        ...
        if (
          $scope.data.apiMetadata.numPages &&
          parseInt($scope.data.apiMetadata.page) >=
            parseInt($scope.data.apiMetadata.numPages)
        ) {
          return;
        }
      ...
      $scope.$watchGroup(
        ["data.filterDateFrom", "data.filterDateTo"],
      ...
      $scope.createPaymentData = function () {
        ...
        RegistrationPaymentsService.create(dataView)
          .success(function (data, status, headers) {
            $scope.data.payments = [];
      ...
      $scope.exportPaymentsFilter = function () {
        ...
        var status = $scope.data.status ? $scope.data.status : null;
        var from = $scope.data.filterDateFrom
          ? moment($scope.data.filterDateFrom).format("YYYY-MM-DD")
          : "";
        var to = $scope.data.filterDateTo
          ? moment($scope.data.filterDateTo).format("YYYY-MM-DD")
          : "";
        var url = MapasCulturais.createUrl("payment", "exportFilter", {
      ...
      $scope.getHistoryPayment = function (payment) {
        RegistrationPaymentsService.getHistory(payment.id).success(function (
          data,
          status,
          headers
        ) {
          ...
          var result = data.revisions.map(function (revision) {
            ...
            revision.createTimestamp.date = moment(
              revision.createTimestamp.date
            ).format("DD/MM/YYYY");
            return revision;
          });
      ...
      $scope.deleteSelectedPayments = function () {
        if (
          !confirm(
            "Você tem certeza que deseja deletar os pagamentos selecionados?"
          )
        ) {
          return;
        }
                ...
                $scope.deletingPayemntTimeOut = setTimeout(function () {
                  if ($scope.data.deletingPayments == 0) {
                    ...
                    MapasCulturais.Messages.success(
                      "Pagamentos deletados com sucesso"
                    );
                  }
                }, 200);
              })
              .error(function () {
                ...
                if ($scope.data.deletingPayments == 0) {
                  ...
                  MapasCulturais.Messages.error(
                    "Erros ocorreram durante o processamento"
                  );
                }
                return;
              });
          ...
          payments.forEach(function (payment, i) {
            setTimeout(function () {
              ...
              if (typeof result.amount == "string") {
                result.amount =
                  result.amount.replace(".", "").replace(",", "") / 100;
              }

              RegistrationPaymentsService.update(result)
                .success(function () {
                  var index = $scope.data.payments.indexOf(payment);
                  ...
                  $scope.editingPayemntTimeOut = setTimeout(function () {
                    if ($scope.data.editingPayments == 0) {
                      ...
                      $scope.$apply();
                      MapasCulturais.Messages.success(
                        "Pagamentos editados com sucesso"
                      );
                    }
                  }, 200);
                })
      ...
      $scope.startEdition = function (payment) {
        ...
        result.amount = new Intl.NumberFormat("pt-BR", {
          style: "currency",
          currency: "BRL",
        })
  ...
  module.factory("RegistrationPaymentsService", [
    "$http",
    "$rootScope",
    "UrlService",
    function ($http, $rootScope, UrlService) {
      return {
        find: function (data) {
          ...
          var url = MapasCulturais.createUrl("payment", "findPayments", {
            opportunity: MapasCulturais.entity.id,
          });
          ...
          return $http
            ...
            .success(function (data, status, headers) {
              for (var i = 0; i < data.length; i++) {
                ...
                data[i].payment_date = new Date(
                  data[i].payment_date + " 12:00"
                );
                data[i].url = url;
              }
            ...
            .error(function (data, status) {
              $rootScope.$emit("error", {
                message: "Payments not found for this opportunity",
                data: data,
                status: status,
              });
            });
        ...
        create: function (payment) {
          var url = MapasCulturais.createUrl("payment", "createMultiple", {
            opportunity: MapasCulturais.entity.id,
          });

          payment.amount =
            payment.amount.replace(".", "").replace(",", "") / 100;
        ...
        getHistory: function (paymentId) {
          var url = MapasCulturais.createUrl("payment", "revision", {
            paymentId: paymentId,
          });

          return $http
            .get(url, { paymentId: paymentId })
            .success(function (data, status, headers) {
              $rootScope.$emit("registration.create", {
                message: "Payments found",
                data: data,
